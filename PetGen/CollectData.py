...
def date_generator(param):
    ...
    if param == 0:
        ...
        age = param * 365 * 24 * 60 * 60
        visit = stime + random.random() * age
        return time.strftime(scheme, time.localtime(visit))
...
def main():
    ...
    for n in range(0, count, 1):
        ...
        breed = image.split(".")[0]
        last_visit = date_generator(age)

        new_pet = Pet(n, image, name, breed, age, last_visit, owner)
        f.write(str(new_pet) + "\n")
